...
### 7. `globalAssets` (Favorites)
- **Purpose**: Stores the user's **Favorites** (user-facing naming; database naming remains `globalAssets`).
- **Key Path**: `id` (string)
...
| `getManagerData` | *(none)* | Retrieves all local adventures, all story cards, all favorites (stored in `globalAssets`), and general settings. |
| `saveGlobalAsset` | `asset: GlobalAsset` | Saves or updates a favorite (global asset) in the local IndexedDB. |
| `deleteGlobalAsset` | `id: string` | Deletes a favorite (global asset) from the local IndexedDB. |
| `importGlobalAsset` | `shortId`, `assetId` | Fetches a favorite (global asset) and applies it to the active play session using GraphQL mutations. |
...
- **Proper Noun Detection**: `runProperNounAutoDetection(shortId, newActions)` (which calls `detectProperNouns(text, knownNames, lexiconNames)`) scans incoming gameplay actions for proper noun candidates using Compromise NLP. 
  - **Turn Preprocessing**: To prevent formatting/symbol noise from corrupting NLP tags, detection pre-processes turn texts by stripping bracketed content `[...]`, braced content `{...}`, parenthesized content `(...)`, and slash commands/tokens (e.g. `/AC`).
  - **Lexicon Feeding**: To ensure universe-specific terminology unique to campaigns (including hyphenated or custom names) is not dropped, it **dynamically feeds the local card index** (all cards fetched for the current campaign via `repo.getAllCards(shortId)`) **and known names** directly into the Compromise scanner as a custom token lexicon layer on every run.
  - **De-stuttering**: Detection first runs `deStutter(text)` to strip AI-Dungeon stutter prefixes (a single letter + hyphen + word starting with the SAME letter, e.g. `m-Management` → `Management`, `H-here` → `here`, looped for `w-w-w-well`) — without it, a stutter breaks NLP tagging and `Management Office` is detected as just `Office`. The same-first-letter constraint preserves real hyphenated terms (`X-Men`, `T-Rex`, `e-mail`, `Kool-Aid`).
  - **POS Tag & Lowercase Split Candidate Filtering**: For single-word candidates, it applies a part-of-speech tag filter: if the candidate's lowercase form matches common verbs, adjectives, adverbs, pronouns, conjunctions, or prepositions (excluding proper nouns), it is ignored. Furthermore, split candidates starting with a lowercase letter (unless present in the custom lexicon) are ignored, and a broadened `ignoreList` filters sentence-starting common words, UI terms, and music metadata terms.
  - **Designation-Extension Pass**: The NLP truncates `Building J`/`Unit B`/`Apartment 4C` to the bare noun because it won't tag a trailing lone letter/number as part of the proper noun, so any detected noun immediately followed by a standalone designator token (`[A-Z]\d{0,2}` or `\d{1,3}[A-Z]?`, e.g. `J`, `B`, `A1`, `4C`) gets it re-attached (a negative lookahead excludes a following name like `Building Justin`; a bare `I` is excluded as the pronoun). The designation extension runs **before** the known-name/sub-name filters inside `detectProperNouns` — otherwise a bare `Building` is dropped as a known key or sub-name of `{protagonist}'s Apartment Building` and `Building J` never forms.
  - **Entity Resolution & Alias-Matching Pass**: Before suggesting a noun or logging it, it performs alias-matching (`isAliasMatch`, logged via `console.info` decisions). **Alias semantics:** a candidate that is a SUB-name of an existing entity (`Blake` ⊆ `Nathaniel Blake`) is always an alias; the reverse (existing ⊆ candidate, i.e. the candidate is MORE specific) counts as an alias only when the existing name is itself distinctive (≥2 words) — so a generic single-word key like `building`/`office`/`unit` does NOT swallow a more-specific noun like `Building J`. It uses a subset-based word comparison (ignoring common titles/linking words) against all active card titles, keys, the protagonist, logged nouns, and pending suggestions to prevent duplicate variants (e.g., "Brother Nathaniel" and "Nathaniel Blake") from causing roster bloat. Hits are recorded in `AdventureMeta.properNounLogs` and surface as pending `locationSuggestions` rendered as banners above the Card Manager roster (`#location-banners-container`) asking the user to classify the noun's type.
...
### L. Adventures Manager (Favorites & DB Explorer)
- **Purpose**: Displays a dedicated screen to browse cached adventure assets and maintain a reusable list of **Favorites** (formerly known as the Global Bucket) containing AI Instructions, Author's Notes, Plot Essentials character bios, and Story Cards that can be duplicated (favorited) and imported into the active adventure.
- **Routing & Display Modes**:
  - **Active Play Page (`/play` or ending in `/play`)**: Accessible inside **Settings → Favorites** tab pane. Fully supports favoriting local assets and importing favorites into the active adventure session via GQL mutations.
  - **Other Pages (except `/settings`)**: Renders in **Manager-Only** mode as a standalone sidebar panel. All active play controls (such as imports) are disabled, focusing purely on managing Favorites and browsing the local IndexedDB cache.
...
- **Favorites CRUD & UI Accordions**: 
  - Users can manually create, edit, or delete Favorites through the UI. New assets can also be favorited from the database explorer.
  - **Type Swapping**: In the Edit Favorite form, changing the Asset Type dropdown dynamically switches field layouts (swapping between standard title/value fields and full Story Card fields), preserving current inputs and updating `asset.type` and `asset.cardType` on save.
  - **Collapsible categories**: The Favorites list view uses collapsible `<details class="group-header">` accordions for each asset type (AIN, AN, PE, SC). Open states are synchronized across panel renders.
- **Database Explorer**: Accordion-style navigation listing all saved adventures in the local DB. Expanding an adventure lists its associated AIN, AN, PE Characters, and Story Cards. Next to each asset, a star button copies the asset into Favorites. Includes a "Delete" button next to each adventure card that triggers a modal overlay directly within the panel.
  - **Category Reordering**: Grouped top-level categories under each adventure card are reordered as: (1) AI Instructions (if present), (2) Plot Essentials (containing character rows and a nested collapsible drawer **"📄 See Full Plot Essentials"** containing the remainder of the raw Plot Essentials string), (3) Author's Note, and (4) Story Cards (grouped by card type and sorted).
  - **Indented Child Accordion Styling**: Nested categories and Story Card type groupings are styled as indented child accordions (`details.local-category-details`) with secondary left-border accent lines and chevrons.
...
- **GraphQL Imports**: Importing an asset uses GraphQL mutations to update active session state on AID servers:
  ...
  - **AI Instructions (`ain`)**: Fetches current AIN, appends the new text, and calls `UpdateAdventureState` (with auto-capture capability when new instructions are loaded).
...
### Manifest MV3 Compatibility & Background Scripts
- **Chrome validation error**: Chrome MV3 strictly forbids `"background.scripts"` in the manifest and throws a loading error if present.
- **Firefox validation error**: Firefox MV3 event-driven pages require `"background.scripts"` (and AMO validator fails if only `"service_worker"` is provided without fallback).
- **Permanent Fix**: Keep the root `manifest.json` clean of `"scripts"` (only `"service_worker"`) so that developer tools are Chrome-friendly. The build script (`build.mjs`) automatically compiles both target directories concurrently in a single run:
  - `dist/chrome/`: Packages Chrome-compatible bundles and a manifest with only `service_worker`.
  - `dist/firefox/`: Packages Firefox-compatible bundles and a manifest with both `service_worker` and `scripts` to pass AMO validation.
- **Loading Unpacked**:
  - In Chrome (`chrome://extensions`): load unpacked from `dist/chrome`.
  - In Firefox (`about:debugging`): load unpacked from `dist/firefox`.
...
- **Build**: `npm run build` (packages Chrome and Firefox bundles into `dist/chrome/` and `dist/firefox/` concurrently).
