...
(() => {
  ...
  function normalizeType(t3) {
    const x = (t3 ?? "character").toLowerCase();
    return STD_TYPES.has(x) ? x : "custom";
  }
  function buildPrompt(req) {
    const s1 = req.customPromptSection1 || DEFAULT_PROMPT_SECTION_1;
    const s22 = req.customPromptSection2 || DEFAULT_PROMPT_SECTION_2;
    const s3 = req.customPromptSection3 || DEFAULT_PROMPT_SECTION_3;
    const s4 = req.customPromptSection4 || DEFAULT_PROMPT_SECTION_4;
    const template = [s1, s22, s3, s4].join("\n\n");
    let system = template.replace(/{protagonist}/g, req.protagonist);
    system += "\n\nNAME FIDELITY: In each proposal, set `name` to the EXACT `name` value of the matching `characters` entry \u2014 not an in-story nickname, first name, or shortened form.";
    system += `

COVERAGE: Independently evaluate EVERY entry in \`characters\` for trackable change \u2014 not only the most prominent character in the scene. Beyond overt personality shifts, a character develops through lasting changes in their relationship toward ${req.protagonist}, their confidence or agency, or their self-understanding \u2014 and a single pivotal scene can be the evidence of such a lasting shift, even for a supporting character. Continue to distinguish a permanent change (propose an update) from a purely transient mood or one-off situational detail (do not log), but do not dismiss real growth merely because it surfaced in one scene.`;
    system += `

PRESENCE: Only update a character who actively participates in this window \u2014 one who does or says something, or is directly involved in events that change them. If a character is merely mentioned, referenced, or named in passing without being present or acting, do NOT update them.`;
    system += `

FORCING FUNCTION: You MUST account for EVERY character in \`characters\`. In \`proposals\`, output exactly one entry per character: set \`action\` to "update" with \`newEntry\` and \`changeSummary\` only if the PRESENCE and COVERAGE rules justify a change; otherwise set \`action\` to "skip" with a brief \`reason\`. Do not omit any listed character and do not invent new ones. Only "update" entries are applied.`;
    const guidance = { ...DEFAULT_TYPE_GUIDANCE, ...req.typeGuidance ?? {} };
    const presentTypes = [...new Set(req.characters.map((c2) => normalizeType(c2.type)))];
    if (presentTypes.length > 0) {
      system += "\n\nPER-TYPE GUIDANCE \u2014 each `characters` entry has a `type`; apply the matching guidance:\n" + presentTypes.map((t3) => `  - ${t3}: ${guidance[t3] ?? ""}`).join("\n");
    }
    if (req.useMemories) {
  ...
  function validateProposals(resp, characters, protagonistName) {
    const norm = (s3) => s3.trim().toLowerCase();
    const resolver = /* @__PURE__ */ new Map();
    for (const c2 of characters) resolver.set(norm(c2.name), { name: c2.name, source: c2.source ?? "card" });
    for (const c2 of characters) {
      for (const a2 of c2.aliases ?? []) {
        const key = norm(a2);
        if (key && !resolver.has(key)) resolver.set(key, { name: c2.name, source: c2.source ?? "card" });
      }
    }
    const proposals = [];
    const warnings = [];
    for (const raw of resp.proposals ?? []) {
      if (raw.action === "skip" || !raw.newEntry || !raw.newEntry.trim()) continue;
      ...
      if (p5.newEntry.length > limit) {
        p5.newEntry = p5.newEntry.slice(0, limit);
        warnings.push(`Entry for ${p5.name} truncated to ${limit} characters.`);
      }
      const tw = triggerWarning(p5);
      if (tw) warnings.push(tw);
      proposals.push(p5);
    }
    return { proposals, warnings };
  }
  async function analyze(provider, req) {
    const resp = await provider.infer(req);
    return validateProposals(resp, req.characters, req.protagonist);
  }
  var COMMON_WORDS, DEFAULT_PROMPT_SECTION_1, DEFAULT_PROMPT_SECTION_2, DEFAULT_PROMPT_SECTION_3, DEFAULT_PROMPT_SECTION_4, DEFAULT_SYSTEM_PROMPT, DEFAULT_TYPE_GUIDANCE, STD_TYPES;
  var init_engine = __esm({
    "src/inference/engine.ts"() {
      "use strict";
      COMMON_WORDS = ["cat", "art", "the", "an", "or", "is", "in", "on", "he", "she", "it", "you", "a"];
      ...
      STD_TYPES = /* @__PURE__ */ new Set(["character", "class", "race", "location", "faction", "memoraid"]);
    }
  });

  // src/inference/card-command.ts
  var card_command_exports = {};
  __export(card_command_exports, {
    DEFAULT_CARD_COMMANDS: () => DEFAULT_CARD_COMMANDS,
    DEFAULT_FORMATTING_MODE: () => DEFAULT_FORMATTING_MODE,
  ...
  function defaultCommandForType(type) {
    return DEFAULT_CARD_COMMANDS[normalizeType(type)] ?? DEFAULT_CARD_COMMANDS.custom;
  }
  function hasTitleToken(template) {
    return /\{\{\s*title\s*\}\}/.test(template);
  }
  ...
  function parseProtagonistName(memory) {
    ...
    const m3 = memory.match(/(?:your name|player name)\s*:\s*([^\n\]]+)/i);
    return m3 ? m3[1].trim() : null;
  }
  var DEFAULT_CARD_COMMANDS, DEFAULT_FORMATTING_MODE;
  var init_card_command = __esm({
    "src/inference/card-command.ts"() {
      "use strict";
      init_engine();
...
- Output nothing before the opening "[" or after the closing "]".`
 ...
 -> X  - '${str}'  : (${form})  `);
  ...
  function openAidDb() {
    ...
    _dbPromise = openDB("aid-tracker", 4, {
      async upgrade(db, oldVersion, _newVersion, tx) {
        ...
        if (oldVersion < 2 && db.objectStoreNames.contains("narrative")) {
          const narrative = await tx.objectStore("narrative").getAll();
          const actions = tx.objectStore("actions");
          for (const rec of narrative) {
            for (const a2 of rec.actions ?? []) await actions.put({ ...a2, shortId: rec.shortId });
          }
          db.deleteObjectStore("narrative");
        }
        if (!db.objectStoreNames.contains("cards")) {
          const s3 = db.createObjectStore("cards", { keyPath: ["shortId", "id"] });
          s3.createIndex("by-shortId", "shortId");
        }
        if (!db.objectStoreNames.contains("versions")) {
          const s3 = db.createObjectStore("versions", { keyPath: "id" });
          s3.createIndex("by-shortId", "shortId");
        }
  ...
  function byCreatedAt(a2, b) {
    ...
    const na = Number(a2.id), nb = Number(b.id);
    if (!Number.isNaN(na) && !Number.isNaN(nb)) return na - nb;
    return a2.id < b.id ? -1 : a2.id > b.id ? 1 : 0;
  }
  var opCache = /* @__PURE__ */ new Map();
  var opCacheInitialized = false;
  async function ensureOpCache() {
    ...
    try {
      ...
      console.error("[AID repo] Failed to initialize op cache:", e2);
    }
  }
  var Repo = class {
    async upsertAdventure(meta) {
      const db = await openAidDb();
    ...
    async putActions(shortId, actions) {
      ...
      for (const a2 of actions) await tx.store.put({ ...a2, shortId });
      await tx.done;
    }
    async deleteAction(shortId, id) {
      const db = await openAidDb();
      await db.delete("actions", [shortId, id]);
    }
    async replaceAllActions(shortId, actions) {
      const db = await openAidDb();
      const tx = db.transaction("actions", "readwrite");
    ...
    async getActions(shortId) {
      const db = await openAidDb();
      const rows = await db.getAllFromIndex("actions", "by-shortId", shortId);
      return rows.map(({ shortId: _s, ...a2 }) => a2).sort(byCreatedAt);
    }
    async getAction(shortId, id) {
      const db = await openAidDb();
      const row = await db.get("actions", [shortId, id]);
      if (!row) return void 0;
      const { shortId: _s, ...a2 } = row;
      return a2;
    }
    async getActionCount(shortId) {
      const db = await openAidDb();
    ...
    async putCards(shortId, cards) {
      const db = await openAidDb();
      const tx = db.transaction("cards", "readwrite");
      for (const c2 of cards) await tx.store.put({ ...c2, shortId });
      await tx.done;
    }
    async getCards(shortId) {
      const db = await openAidDb();
      return db.getAllFromIndex("cards", "by-shortId", shortId);
    }
    async getAllCards(shortId) {
      const db = await openAidDb();
      return db.getAllFromIndex("cards", "by-shortId", shortId);
    }
    ...
    async reconcileDeletedCards(shortId, presentIds) {
      const db = await openAidDb();
      const present2 = new Set(presentIds);
      const tx = db.transaction("cards", "readwrite");
      let cursor = await tx.store.index("by-shortId").openCursor(shortId);
      while (cursor) {
        const c2 = cursor.value;
        if (!present2.has(c2.id) && c2.deletedAt == null) {
    ...
    async getVersions(shortId) {
      const db = await openAidDb();
      return (await db.getAllFromIndex("versions", "by-shortId", shortId)).sort((a2, b) => a2.createdAt < b.createdAt ? -1 : a2.createdAt > b.createdAt ? 1 : 0);
    }
    async setVersionStatus(id, status) {
      const db = await openAidDb();
      const v2 = await db.get("versions", id);
      if (v2) await db.put("versions", { ...v2, status });
    }
    async setVersionPushed(id, pushedAt) {
      const db = await openAidDb();
      const v2 = await db.get("versions", id);
      if (v2) await db.put("versions", { ...v2, pushedAt });
    }
    async getSettings() {
      const db = await openAidDb();
      const rec = await db.get("settings", "singleton");
...
Notable Items: specific permanent contents. You must preserve the literal names of specific books, exact instrument models, and unique trophies from the source text; never generalize or substitute them with generic placeholders. Keep the entry high-density and well under the absolute emergency ceiling of 2,000 characters; do not pad. Prune transient scene recaps, story events, and redundant decorative wording, but PRESERVE the enduring flavor that defines the place \u2014 its atmosphere, social fabric, and narrative role are required content, not fluff. The entry must serve as both a spatial and a narrative guide for the AI engine.`
        ...
        if (HISTORICAL_LOCATION_DEFAULTS.includes(settings.cardCommands.location)) {
          ...
          const { DEFAULT_CARD_COMMANDS: DEFAULT_CARD_COMMANDS2 } = await Promise.resolve().then(() => (init_card_command(), card_command_exports));
          settings.cardCommands.location = DEFAULT_CARD_COMMANDS2.location;
          await this.setSettings(settings);
        }
      }
      return settings;
    }
    async setSettings(s3) {
      const db = await openAidDb();
      await db.put("settings", { ...s3, _k: "singleton" });
    }
    ...
    async exportAll() {
      const db = await openAidDb();
      const stores = {};
      for (const s3 of BACKUP_STORES) {
        const rows = await db.getAll(s3).catch(() => []);
        stores[s3] = s3 === "settings" ? rows.map(({ apiKeys, ...rest }) => rest) : rows;
      }
      return { __aidBackup: true, dbVersion: 4, exportedAt: (/* @__PURE__ */ new Date()).toISOString(), stores };
    }
    ...
    async importAll(data) {
      ...
      const db = await openAidDb();
      const counts = {};
      const hasKeys = (o2) => o2?.apiKeys && Object.keys(o2.apiKeys).length > 0;
      for (const s3 of BACKUP_STORES) {
        const rows = data.stores[s3];
        if (!Array.isArray(rows) || rows.length === 0) continue;
        const existingSettings = s3 === "settings" ? await db.get("settings", "singleton").catch(() => null) : null;
        const tx = db.transaction(s3, "readwrite");
        let n3 = 0;
        for (const row of rows) {
          const toPut = s3 === "settings" ? { ...row, apiKeys: hasKeys(existingSettings) ? existingSettings.apiKeys : row.apiKeys ?? {} } : row;
          try {
    ...
    async getGlobalAsset(id) {
      const db = await openAidDb();
      return db.get("globalAssets", id);
    }
    async putGlobalAsset(asset) {
      const db = await openAidDb();
      await db.put("globalAssets", asset);
    }
    async deleteGlobalAsset(id) {
      const db = await openAidDb();
  ...
  // src/sync/gameplay-fetch.ts
  function buildGameplayRequest(endpoint, query, shortId, token, limit) {
    return {
      url: endpoint,
  ...
  async function backfillAll(fetcher, onProgress, maxPages) {
    const byId = /* @__PURE__ */ new Map();
    let cursor = null;
    for (let i3 = 0; i3 < maxPages; i3++) {
      const page = await fetcher(cursor);
      for (const a2 of page.actions) byId.set(a2.id, a2);
      onProgress?.(byId.size);
      if (!page.hasMore) break;
      cursor = page.nextCursor;
      if (cursor == null) break;
    }
    return [...byId.values()].sort((a2, b) => {
      const ta = a2.createdAt ?? "", tb = b.createdAt ?? "";
      if (ta !== tb) return ta < tb ? -1 : 1;
      const na = Number(a2.id), nb = Number(b.id);
      return !Number.isNaN(na) && !Number.isNaN(nb) ? na - nb : a2.id < b.id ? -1 : 1;
    });
  }

  // src/sync/reconcile.ts
  function toCanonical(a2) {
    return { id: a2.id, text: a2.text, type: a2.type, createdAt: a2.createdAt, updatedAt: a2.updatedAt };
  }
  function isLive(a2) {
    return a2.undoneAt == null && a2.deletedAt == null;
  }
  function diffActionUpdate(payload) {
    const upserts = [];
    const removeIds = [];
    for (const a2 of payload.actions) {
      if (isLive(a2)) upserts.push(toCanonical(a2));
      else removeIds.push(a2.id);
    }
    return { upserts, removeIds };
  }
  function applyActionUpdate(current, payload) {
    const result = [...current];
    const indexById = new Map(result.map((a2, i3) => [a2.id, i3]));
    for (const raw of payload.actions) {
      ...
      if (!isLive(raw)) {
        if (at !== void 0) {
          result.splice(at, 1);
          indexById.clear();
          result.forEach((a2, i3) => indexById.set(a2.id, i3));
        }
        continue;
      }
      if (at !== void 0) {
  ...
  function parsePlotEssentials(memory) {
    ...
    const re = /\[([^\]]+)\]|\{([^\}]+)\}/g;
    let m3;
    while ((m3 = re.exec(memory)) !== null) {
      const content = m3[1] !== void 0 ? m3[1] : m3[2];
      const info = blockName(content);
      if (info) blocks.push({ name: info.name, text: content.trim(), isPlayer: info.isPlayer });
    }
    return blocks;
  }
  function parseMemories(memory) {
    if (!memory) return null;
    const re = /\[\s*(Memories\s*\(newest\s*to\s*oldest\):[\s\S]*?)\]/gi;
    const m3 = re.exec(memory);
    return m3 ? m3[1].trim() : null;
  }
  function replaceBlock(memory, name, newEntry) {
    if (!memory) return null;
    const targetName = name.trim().toLowerCase();
    if (targetName === "memories") {
      const re2 = /\[\s*(Memories\s*\(newest\s*to\s*oldest\):[\s\S]*?)\]/gi;
      const m4 = re2.exec(memory);
      if (m4) {
        return memory.slice(0, m4.index) + `[${newEntry}]` + memory.slice(m4.index + m4[0].length);
      }
    }
    const re = /\[([^\]]+)\]|\{([^\}]+)\}/g;
    let m3;
    while ((m3 = re.exec(memory)) !== null) {
      const isBrace = m3[2] !== void 0;
      const content = isBrace ? m3[2] : m3[1];
      const info = blockName(content);
      if (info && info.name.trim().toLowerCase() === targetName) {
        const wrapperOpen = isBrace ? "{" : "[";
        const wrapperClose = isBrace ? "}" : "]";
        return memory.slice(0, m3.index) + `${wrapperOpen}${newEntry}${wrapperClose}` + memory.slice(m3.index + m3[0].length);
      }
    }
    return null;
  }
...
`;
    ...
    for (const c2 of cards) {
      if (c2.id === card.id || c2.deletedAt || (c2.type || "").toLowerCase() !== "location" || !c2.value) continue;
      const names = [c2.title || "", ...(c2.keys || "").split(/[,;]+/)].map((s3) => s3.trim().toLowerCase()).filter((s3) => s3.length >= 3);
      if (!names.some((n3) => searchSpace.includes(n3))) continue;
      const chunk = `Containing location "${c2.title || c2.keys}" (context only \u2014 do not merge into this entry):
${c2.value.slice(0, 600)}
...
`;
  ...
  function buildAnalyzeRequest(protagonist, recentActions, memory) {
    const narrative = recentActions.map((a2) => a2.text).join("\n");
    const characters = parsePlotEssentials(memory).map((b) => {
      const first = b.name.trim().split(/\s+/)[0];
      return { name: b.name, currentEntry: b.text, source: "plot", type: "character", aliases: first && first !== b.name ? [first] : [] };
    });
    return { protagonist, present: characters.map((c2) => c2.name), narrative, characters };
  }
  ...
  function stripFences(text) {
    const m3 = text.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
    return (m3 ? m3[1] : text).trim();
  }
  function extractFirstTextBlock(json) {
  ...
  function extractBalancedObject(text) {
    ...
    if (start2 === -1 || end2 === -1 || end2 <= start2) return null;
    const candidate = text.slice(start2, end2 + 1);
  ...
  function buildModelsRequest(apiKey) {
    return {
  ...
  function parseModelsResponse(json) {
    ...
    return Array.isArray(data) ? data.map((m3) => m3?.id).filter((x) => typeof x === "string").filter((id) => id !== "claude-fable-5") : [];
  }
  async function listModels(apiKey) {
    const r2 = buildModelsRequest(apiKey);
    const res = await fetch(r2.url, { headers: r2.headers });
  ...
  var ClaudeProvider = class {
    ...
    async complete(system, user, cachePrefix) {
      ...
      const u2 = json?.usage;
      if (u2) {
        console.info(`[AID claude] usage: input=${u2.input_tokens} cache_write=${u2.cache_creation_input_tokens ?? 0} cache_read=${u2.cache_read_input_tokens ?? 0} output=${u2.output_tokens}`);
      }
      const text = extractFirstTextBlock(json);
  ...
  // src/inference/openai.ts
  init_engine();
  function stripFences2(text) {
    const m3 = text.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
    return (m3 ? m3[1] : text).trim();
  }
  function tryParseProposals2(s3) {
    try {
      const parsed = JSON.parse(s3);
      if (parsed && typeof parsed === "object" && "proposals" in parsed) {
        return { proposals: Array.isArray(parsed.proposals) ? parsed.proposals : [] };
  ...
  function extractBalancedObject2(text) {
    const start2 = text.indexOf("{");
    const end2 = text.lastIndexOf("}");
    if (start2 === -1 || end2 === -1 || end2 <= start2) return null;
    const candidate = text.slice(start2, end2 + 1);
  ...
  function parseOpenAIResponse(text) {
    const fenceStripped = stripFences2(text);
    const fromFence = tryParseProposals2(fenceStripped);
    if (fromFence) return fromFence;
    const balanced = extractBalancedObject2(text);
    if (balanced) {
      const fromBalanced = tryParseProposals2(balanced);
      if (fromBalanced) return fromBalanced;
    }
    return { proposals: [] };
  }
  ...
  async function listOpenAIModels(apiKey) {
    ...
    if (!res.ok) return [];
    const json = await res.json();
    return Array.isArray(json.data) ? json.data.map((m3) => m3.id).filter((id) => id.includes("gpt-") || id.includes("o1-") || id.includes("o3-")).sort() : [];
  }

  // src/inference/gemini.ts
  init_engine();
  function stripFences3(text) {
    const m3 = text.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
    return (m3 ? m3[1] : text).trim();
  }
  function tryParseProposals3(s3) {
    try {
      const parsed = JSON.parse(s3);
      if (parsed && typeof parsed === "object" && "proposals" in parsed) {
        return { proposals: Array.isArray(parsed.proposals) ? parsed.proposals : [] };
  ...
  function extractBalancedObject3(text) {
    const start2 = text.indexOf("{");
    const end2 = text.lastIndexOf("}");
    if (start2 === -1 || end2 === -1 || end2 <= start2) return null;
    const candidate = text.slice(start2, end2 + 1);
  ...
  function parseGeminiResponse(text) {
    const fenceStripped = stripFences3(text);
    const fromFence = tryParseProposals3(fenceStripped);
    if (fromFence) return fromFence;
    const balanced = extractBalancedObject3(text);
    if (balanced) {
      const fromBalanced = tryParseProposals3(balanced);
      if (fromBalanced) return fromBalanced;
    }
    return { proposals: [] };
  }
  async function geminiGenerate(apiKey, modelName, system, user, generationConfig) {
    const isGemma = modelName.toLowerCase().startsWith("gemma");
    const gemmaNeedsFold = isGemma;
    const GEMMA_FORMAT_GUARD = '\n\nFORMAT (MANDATORY): Output ONLY what the instructions above specify, and nothing else. Write plain text \u2014 NO Markdown, NO asterisks (*), NO bullet characters other than a literal "- ", NO bold, NO headings, NO indentation. Do NOT restate, label, or narrate the prompt, the scene, the character, or your task \u2014 never output lines like "Character:", "Goal:", "Latest Action:", or "Stimulus:".';
    const userText = isGemma ? `${user}${GEMMA_FORMAT_GUARD}` : user;
    const body = gemmaNeedsFold ? { contents: [{ parts: [{ text: `${system}
...
${userText}` }] }], generationConfig } : { contents: [{ parts: [{ text: userText }] }], systemInstruction: { role: "system", parts: [{ text: system }] }, generationConfig };
  ...
  async function listGeminiModels(apiKey) {
    ...
    const json = await res.json();
    return Array.isArray(json.models) ? json.models.filter((m3) => {
      const methods17 = m3?.supportedGenerationMethods;
      return !Array.isArray(methods17) || methods17.includes("generateContent");
    }).map((m3) => (m3?.name || "").replace("models/", "")).filter((id) => id.startsWith("gemini-") || id.startsWith("gemma-")).filter((id) => !id.includes("robotics")).sort() : ["gemini-1.5-pro", "gemini-1.5-flash"];
  }
  ...
  function tryParseProposals4(s3) {
    try {
      const parsed = JSON.parse(s3);
      if (parsed && typeof parsed === "object" && "proposals" in parsed) {
        return { proposals: Array.isArray(parsed.proposals) ? parsed.proposals : [] };
  ...
  function extractBalancedObject4(text) {
    const start2 = text.indexOf("{");
    const end2 = text.lastIndexOf("}");
    if (start2 === -1 || end2 === -1 || end2 <= start2) return null;
    const candidate = text.slice(start2, end2 + 1);
  ...
  function parseOllamaResponse(text) {
    const fenceStripped = stripFences4(text);
    const fromFence = tryParseProposals4(fenceStripped);
    if (fromFence) return fromFence;
    const balanced = extractBalancedObject4(text);
    if (balanced) {
      const fromBalanced = tryParseProposals4(balanced);
      if (fromBalanced) return fromBalanced;
    }
    return { proposals: [] };
  }
  ...
  async function listOllamaModels(endpoint) {
    ...
    try {
      ...
      const json = await res.json();
      return Array.isArray(json.models) ? json.models.map((m3) => m3.name).sort() : ["llama3", "mistral", "gemma"];
    } catch {
      return ["llama3", "mistral", "gemma"];
    }
  }

  // src/inference/writeback.ts
  function buildGraphQLMutation(endpoint, queryOrOps, token, operationName, variables) {
    const ops = Array.isArray(queryOrOps) ? queryOrOps : [{ operationName, query: queryOrOps, variables }];
    return {
      url: endpoint,
      headers: { "content-type": "application/json", authorization: token },
  ...
  function buildCardSave(endpoint, query, token, card, newValue) {
  ...
  function buildCardCreate(endpoint, query, token, card, newValue) {
  ...
  function buildMemorySave(endpoint, query, token, shortId, newMemory, authorsNote = "") {
  ...
  function buildUpdateAdventureState(endpoint, query, token, shortId, instructionsCustomText, storySummaryText = "", storyCardStoryInformation = "", storyCardInstructions = "") {
  ...
  function buildEditMemory(endpoint, query, token, adventureId, actionId, text) {
  ...
  // src/shared/offmeta-parser.ts
  var SECTIONS_MAP = {
    "Premade AIN": "\u{1F916} Premade AIN",
    "AN/PE": "\u{1F916} AN/PE",
    "POV & Tense": "\u{1F4F9} POV & Tense",
    "Writing Direction": "\u{1F4DD} Writing Direction",
    "Description Weeding": "\u{1F331} Description Weeding",
    "Pacing": "\u23F1\uFE0F Pacing",
    "Coherence": "\u{1FAE0} Coherence",
    "Removing Repetition": "\u{1F501} Removing Repetition",
    "Push Lines": "\u{1FAF8} Push Lines",
    "Characterization": "\u{1F603} Characterization",
    "Arguing Problem": "\u{1F621} Arguing Problem",
    "Romance": "\u{1F970} Romance",
    "Character Controls": "\u{1F3AE} Character Controls",
    "Combat & Difficulty": "\u2694\uFE0F Combat & Difficulty",
    "Name Fixing": "\u{1F92C} Name Fixing",
    "Fantasy": "\u{1F3F0} Fantasy",
    "Scifi": "\u{1F916} Scifi",
    "Miscellaneous": "\u{1F937} Miscellaneous"
  };
  var IGNORED_SECTIONS = [
    "Suggested Settings",
    "Trope Killer Dictionary",
    "Commands/SSCs",
    "Introduction"
  ];
  function isAinSubheading(trimmed) {
  ...
  function parseOffMetaText(text) {
    const sections = [];
    ...
    const closeBlock = () => {
      if (inBlock && blockLines.length > 0 && currentSection && currentGroup) {
        ...
        if (content) {
          const isDescription = currentSection.title === "\u{1F916} AN/PE" && (content.includes("suggest keeping it simple") || content.includes("should usually only contain raw information"));
          if (!isDescription) {
    ...
    for (let i3 = 0; i3 < lines.length; i3++) {
      ...
      if (currentSection.title === "\u{1F916} Premade AIN") {
        if (!trimmed) continue;
      ...
      } else if (currentSection.title === "\u{1F916} AN/PE") {
        const isHeading = (t3) => {
          const norm = t3.toLowerCase().replace(/['’]/g, "");
          return norm === "authors note" || norm === "authors note:" || norm === "basic authors note" || norm === "basic authors note:" || norm === "plot essentials" || norm === "plot essentials:";
        };
    ...
    return sections.filter((s3) => s3.groups.some((g4) => g4.items.length > 0));
  }
  ...
  function buildMemoNotes(notes, maxChars = 9e3) {
    if (!notes.thoughtLog.length) return "";
    let log3 = [...notes.thoughtLog];
    while (log3.length > 0) {
      const body = log3.map((e2) => `(turn ${e2.turn})${e2.action ? ` Trigger: ${e2.action}` : ""}
${e2.text}`).join("\n\n");
      const full = `${LOG}
${body}`;
  ...
  function pushThought(log3, entry, maxEntries = 15) {
  ...
  function thoughtsSince(log3, sinceTurn, maxChars) {
    const parts = [];
    let len = 0;
    for (const e2 of log3) {
      if (e2.turn <= sinceTurn) continue;
      const piece = e2.action ? `${e2.action}
${e2.text}` : e2.text;
...
{${body}}
  ...
  function buildThoughtContext(log3, n3, name, maxChars) {
    return renderThoughtBlock(log3, n3, `${name}'s recent thoughts (oldest to newest):`, "oldest-first", maxChars);
  }
  ...
  var OFFMETA_FETCH_COOLDOWN_MS = 5 * 60 * 1e3;
  async function fetchOffMetaRepositoryIfNeeded() {
    const now = Date.now();
  ...
  var autoUpdateInFlight = /* @__PURE__ */ new Set();
  function deStutter(text) {
    let prev;
    let out2 = text;
    do {
      prev = out2;
      out2 = out2.replace(/\b([A-Za-z])-([A-Za-z])/g, (m3, a2, b) => a2.toLowerCase() === b.toLowerCase() ? b : m3);
    } while (out2 !== prev);
    return out2;
  }
  function detectProperNouns(text, knownNames, lexiconNames = []) {
    ...
    for (const trimmed of allLexicon) {
      if (trimmed) {
        lexicon4[trimmed] = "ProperNoun";
        if (trimmed.includes("-")) {
          lexicon4[trimmed.replace(/-/g, " ")] = "ProperNoun";
        }
      }
    }
  ...
  function parseLocationFromMemory(memory) {
    if (!memory) return null;
    const re = /(?:\[|\{)\s*(?:Current|Active)\s+Location:\s*([^\]\}]*)(?:\]|\})/i;
    const match2 = re.exec(memory);
    return match2 ? match2[1].trim() : null;
  }
  function updatePlotEssentialsLocation(memory, locationTitle) {
    const mem = memory || "";
    const re = /\[\s*(?:Current|Active)\s+Location:\s*[^\]]*\]|\{\s*(?:Current|Active)\s+Location:\s*[^\}]*\}/gi;
    const match2 = re.exec(mem);
    const newBlock = locationTitle ? `[Current Location: ${locationTitle}]` : "";
    if (match2) {
      if (newBlock) {
        return mem.slice(0, match2.index) + newBlock + mem.slice(match2.index + match2[0].length);
      } else {
        const before2 = mem.slice(0, match2.index);
        const after2 = mem.slice(match2.index + match2[0].length);
        return (before2.trimEnd() + "\n" + after2.trimStart()).trim();
      }
    } else {
      if (newBlock) {
  ...
  function mergeTriggerKey(keys, noun) {
    const add = (noun || "").trim();
    if (!add) return keys || "";
    const existing = (keys || "").split(/[,;]+/).map((k2) => k2.trim()).filter(Boolean);
    if (existing.some((k2) => k2.toLowerCase() === add.toLowerCase())) return keys || "";
    return existing.length ? `${(keys || "").trim().replace(/[,;\s]+$/, "")}, ${add}` : add;
  }
  function isAliasMatch(nameA, nameB) {
    const titlesAndArticles = /* @__PURE__ */ new Set([
      "brother",
      "sister",
      "father",
      "mother",
    ...
    const getNormalizedWords = (name) => {
      return name.toLowerCase().replace(/[-\/\\^$*+?.()|[\]{}]/g, " ").split(/\s+/).map((w) => w.trim()).filter((w) => w.length > 0 && !titlesAndArticles.has(w));
    };
    const wordsA = getNormalizedWords(nameA);
    const wordsB = getNormalizedWords(nameB);
    if (wordsA.length === 0 || wordsB.length === 0) return false;
    const isSubset = (setSmall, setLarge) => {
  ...
  async function runProperNounAutoDetection(shortId, newActions) {
    ...
    for (const c2 of cards) {
      ...
      if (c2.title) existingNames.push(c2.title);
      if (c2.keys) {
        const parts = c2.keys.split(/[,;]+/).map((k2) => k2.trim()).filter(Boolean);
    ...
    for (const c2 of allCards) {
      if (c2.deletedAt) continue;
      if (c2.title) globalLexiconNames.push(c2.title);
      if (c2.keys) {
        const parts = c2.keys.split(/[,;]+/).map((k2) => k2.trim()).filter(Boolean);
  ...
  async function runBackfill(shortId) {
    ...
    `;
      ...
      const memRes = await fetch2(memReq.url, { method: "POST", headers: memReq.headers, body: memReq.body });
    ...
    let backfillCardIds = null;
    const fetcher = async (_cursor) => {
      const req = buildGameplayRequest(endpoint, query, shortId, token, 1e5);
      const res = await fetch2(req.url, { method: "POST", headers: req.headers, body: req.body });
      const json = await res.json();
      const parsed = parseGameplayResponse(json);
      const advUpdate = { shortId };
    ...
    const ordered = await backfillAll(fetcher, void 0, 1);
  ...
  var seedBaselinesInFlight = /* @__PURE__ */ new Map();
  async function seedBaselines(shortId, ctx) {
    ...
    const promise = (async () => {
      ...
      for (const v2 of versions) {
        const isBaseline = v2.changeSummary && v2.changeSummary.startsWith("Baseline");
        if (isBaseline) {
          const key = `${v2.characterName.toLowerCase()}::${v2.cardId || ""}`;
          if (baselineKeys.has(key)) {
            versionsToDelete.push(v2);
          } else {
            baselineKeys.add(key);
            versionsToKeep.push(v2);
          }
        } else {
          versionsToKeep.push(v2);
        }
      ...
      const activeCardNames = new Set(cards.filter((c2) => c2.deletedAt == null).map((c2) => c2.title || c2.keys));
      for (const v2 of versions) {
        if (v2.source === "plot" && activeCardNames.has(v2.characterName)) {
          v2.source = "card";
          if (v2.changeSummary === "Baseline (Plot Essentials)") {
            v2.changeSummary = "Baseline (Story Card)";
          }
          const match2 = cards.find((c2) => c2.deletedAt == null && (c2.title === v2.characterName || c2.keys === v2.characterName));
          if (match2) {
            v2.cardId = match2.id;
      ...
      for (const v2 of versions) {
        const cid = v2.cardId;
        if (!cid || v2.source !== "card") continue;
      ...
      for (const v2 of versions) {
        if (v2.cardId) {
          seenKeys.add(`id:${v2.cardId}`);
        }
        const typeLabel = v2.cardType || "";
        if (typeLabel) {
          seenKeys.add(`name-type:${v2.characterName.trim().toLowerCase()}:${typeLabel.trim().toLowerCase()}`);
        }
      ...
      for (const e2 of entries) {
        ...
        if (!isSeen) {
          ...
          if (e2.type) {
            seenKeys.add(`name-type:${e2.name.trim().toLowerCase()}:${e2.type.toLowerCase()}`);
          }
          seenKeys.add(`name:${e2.name.trim().toLowerCase()}`);
        }
      }
      return versions;
    })();
  ...
  async function runAnalyze(shortId) {
    ...
    for (const p5 of proposals) {
      ...
      await repo.putVersion({
        ...
        entry: p5.newEntry,
        changeSummary: p5.changeSummary,
        triggers: p5.suggestedTriggers,
        source: p5.source,
        status: "pending",
        createdAt: (/* @__PURE__ */ new Date()).toISOString(),
    ...
    return {
      ...
      debug: {
        ...
        characters: req.characters.map((c2) => c2.name),
  ...
  function applyFormattingMode(text, mode) {
...
`;
    ...
    const maxLen = providerName === "ollama" ? 8e3 : providerName === "gemini" ? 7e3 : 16e3;
    const availableLen = Math.max(2e3, maxLen - baseContext.length);
    let contextParts = [];
    let charCount = 0;
    for (let i3 = recentActions.length - 1; i3 >= 0; i3--) {
      const act = recentActions[i3];
      if (!act) continue;
...
${val}
  ...
  async function checkLookbackAutoUpdates(shortId, newActions) {
    ...
    const currentActions = sliceLastActions(allActions, lookbackSize);
    const currentText = currentActions.map((a2) => a2.text || "").join(" ").toLowerCase();
    const activeCharacters = cards.filter((c2) => !c2.deletedAt && c2.type.toLowerCase() === "character");
    const toUpdateCards = [];
    for (const card of fellOutCards) {
      const name = card.title || card.keys;
      if (!name) continue;
      const charVersions = versions.filter((v2) => v2.cardId === card.id || v2.characterName === name);
      const sortedVersions = [...charVersions].sort((a2, b) => {
        const timeA = a2.createdAt || "";
      ...
      const lastUpdateActionCount = charVersions.reduce((max3, v2) => Math.max(max3, v2.actionCount ?? 0), 0);
    ...
    const decisions = [];
    for (const card of activeCharacters) {
      if (fellOutCards.some((c2) => c2.id === card.id)) continue;
      const name = card.title || card.keys;
      if (!name) continue;
    ...
    console.info(
      `[AID bg] Lookback check @ ${totalActionsCount} actions (window ${lookbackSize}): fellOut=[${fellOutCards.map((c2) => c2.title || c2.keys).join(", ")}] ` + (decisions.length ? decisions.join("; ") : "(no character cards)")
    );
  ...
  function capitalizeWords(s3) {
    return s3.split(/\s+/).map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
  }
  var PROFILE_FIELD_RE = /(?:^|\r?\n)[ \t]*[*\-]?[ \t]*(Appearance|Personality|Psychology|Worldview|Quirks|Voice|Goals|Dynamic)[ \t]*[:(]/gi;
  function looksLikeCharacterProfile(text) {
  ...
  function isPlaceholderOrGarbageResponse(text) {
    ...
    return placeholders.some((p5) => lowerText.includes(p5));
  }
  function extractThoughtLoop(text) {
    ...
    while ((m3 = re.exec(text)) !== null) {
      const key = m3[1][0].toUpperCase() + m3[1].slice(1).toLowerCase();
      const val = m3[2].trim().replace(/^\*+\s*/, "");
      if (currentLoop[key] !== void 0) {
    ...
    for (const loop of loops) {
      const parts = order.filter((k2) => loop[k2]).map((k2) => `- ${k2}: ${loop[k2]}`);
      if (parts.length >= 2) {
...
${inner}
  ...
  var gameplayTurnCheckTimers = /* @__PURE__ */ new Map();
  function debouncedGameplayTurnCheck(shortId, upserts) {
  ...
  async function regenerateMemoryBlock(shortId, index3) {
    ...
    if (index3 < 0 || index3 >= memories.length) {
    ...
    if (targetActionIds.size > 0) {
      targetActions = allActions.filter((a2) => targetActionIds.has(a2.id));
    } else {
      if (index3 === memories.length - 1) {
        const summarizedIds = /* @__PURE__ */ new Set();
        for (let i3 = 0; i3 < memories.length - 1; i3++) {
          const m3 = memories[i3];
          if (m3 && m3.actionIds) {
            for (const id of m3.actionIds) summarizedIds.add(id);
          }
        }
        targetActions = allActions.filter((a2) => !summarizedIds.has(a2.id));
      }
    }
...
${targetActions.map((a2) => a2.text || "").join("\n")}`;
    ...
    const newActionIds = targetActions.map((a2) => a2.id);
    const lastActId = targetActions[targetActions.length - 1].id;
    updatedMemories[index3] = {
      ...oldMemory,
      text: cleaned,
      ...
      switch (msg.kind) {
        case "openPermissionsPage": {
          try {
            ...
            console.error("[AID bg] Failed to open permissions page:", e2);
            ...
            if (importantNames && importantNames.length > 0) {
              ...
              for (const impName of importantNames) {
                const baseCard = cards.find((c2) => {
                  ...
                  if (c2.deletedAt) return false;
                  if ((c2.title || "").toLowerCase().endsWith(" (memory)")) return false;
                  const titleLower2 = (c2.title || "").toLowerCase();
                  const keysList = (c2.keys || "").split(/[,;]+/).map((k2) => k2.trim().toLowerCase()).filter(Boolean);
                  if (titleLower2 === impName || keysList.includes(impName)) return true;
                  if (titleLower2.includes(" and ") || titleLower2.includes(" & ")) {
                    const parts = titleLower2.split(/\s+(?:and|&)\s+/);
                    if (parts.includes(impName)) return true;
                  }
                  for (const k2 of keysList) {
                    if (k2.includes(" and ") || k2.includes(" & ")) {
              ...
              const triggered = charactersToCheck.filter(
        ...
        case "refineMemoryBlock": {
          try {
            ...
            console.error("[AID bg] refineMemoryBlock failed:", e2);
            return { error: e2?.message || String(e2) };
          }
        }
        ...
        case "respondToProperNounSuggestion": {
          try {
            ...
            const suggestions = adv.locationSuggestions || [];
            const logs = adv.properNounLogs || [];
            const index3 = suggestions.findIndex((s3) => s3.properNoun.toLowerCase() === properNoun.toLowerCase());
            if (index3 === -1) return { error: "Suggestion not found" };
            const suggestion = suggestions[index3];
            suggestions.splice(index3, 1);
            if (accept) {
        ...
        case "linkProperNounToCard": {
          try {
            ...
            const suggestions = (adv.locationSuggestions || []).filter(
              (s3) => s3.properNoun.toLowerCase() !== properNoun.toLowerCase()
            );
            ...
            const existingLog = logs.find((l2) => l2.properNoun.toLowerCase() === properNoun.toLowerCase());
            if (existingLog) {
        ...
        case "applyOffMetaInstruction": {
          ...
          `;
            ...
            const fetchRes = await fetch2(fetchReq.url, { method: "POST", headers: fetchReq.headers, body: fetchReq.body });
            if (!fetchRes.ok) {
...
${toAppend}` : toAppend;
        ...
        case "adventureMeta": {
          const update = { shortId: msg.shortId };
          if (msg.title !== void 0) update.title = msg.title;
          if (msg.memory !== void 0) {
        ...
        case "setVersionStatus":
          ...
          if (msg.status === "applied") {
            ...
            try {
              ...
              console.error("[AID bg] auto-push threw:", e2);
              return { error: e2?.message || String(e2) };
            }
          }
          return;
        ...
        case "listModels": {
          ...
          try {
            if (providerName === "openai") {
              ...
              return { models: await listModels(key || "") };
            }
          } catch (e2) {
            console.error(`[AID bg] listModels failed for ${providerName}:`, e2);
            return { models: [] };
          }
        }
        case "getState": {
          ...
          console.log(`[AID bg] getState - Total cards in DB: ${cards.length}, Active: ${cards.filter((c2) => !c2.deletedAt).length}`);
          console.log(`[AID bg] getState - Card titles:
` + cards.map((c2) => `- ${c2.title || c2.keys} (${c2.type}, deleted: ${!!c2.deletedAt})`).join("\n"));
          console.log(`[AID bg] getState - Ops:`, ops.map((o2) => `${o2.operationName}`));
          const gameplayOp = ops.find((o2) => o2.operationName === "GetGameplayAdventure");
          if (gameplayOp) {
          ...
          const versionsDebug = versions.map((v2) => {
            let card = cards.find((c2) => c2.title && c2.title.trim().toLowerCase() === v2.characterName.trim().toLowerCase());
            if (!card) {
              card = cards.find((c2) => {
                const keysList = (c2.keys || "").split(/[,;]+/).map((k2) => k2.trim().toLowerCase()).filter(Boolean);
                return keysList.includes(v2.characterName.trim().toLowerCase());
            ...
            return `- ${v2.characterName} (status: ${v2.status}, source: ${v2.source}, cardType: ${type})`;
          }).join("\n");
          console.log(`[AID bg] getState - Versions in DB:
` + versionsDebug);
          const cardCounts = {};
          for (const c2 of cards.filter((c3) => !c3.deletedAt)) {
            cardCounts[c2.type] = (cardCounts[c2.type] || 0) + 1;
          }
          ...
          for (const c2 of cards) {
            const keysList = (c2.keys || "").split(/[,;]+/).map((k2) => k2.trim().toLowerCase()).filter(Boolean);
            for (const k2 of keysList) {
              cardTypeByName.set(k2, c2.type || "character");
            }
            const fullKey = (c2.title || c2.keys || "").trim().toLowerCase();
            if (fullKey) {
              cardTypeByName.set(fullKey, c2.type || "character");
            }
          }
          for (const c2 of cards) {
            if (c2.title) {
              cardTypeByName.set(c2.title.trim().toLowerCase(), c2.type || "character");
            }
          }
          ...
          const typeLabelFor = (nm) => {
            const lower = nm.trim().toLowerCase();
            const t3 = cardTypeByName.get(lower);
            if (t3) return TYPE_LABELS[t3.toLowerCase()] ?? t3;
            if (plotNames.has(lower)) return "Plot Essentials";
            return "Other";
          };
          const versionsByGroup = {};
          for (const v2 of versions) {
            const lbl = typeLabelFor(v2.characterName);
          ...
          const keyStatus = {};
          if (settings?.apiKeys) {
            for (const [k2, v2] of Object.entries(settings.apiKeys)) {
              keyStatus[k2] = !!v2;
            }
          }
          ...
          return {
            ...
            settings: settings ? {
              ...
              // opt-out: checked unless explicitly disabled
        ...
        case "getHiddenAdventures": {
          try {
            ...
            const hidden = (adventures || []).filter((a2) => !!a2.hidden);
...
${blockText}` : blockText;
      ...
      console.error("[AID bg]", msg.kind, e2);
      throw e2;
    }
  }
})();
